...
function hideFormAndShowMessege() {
  ...
  if ( namE.length >=4 && email && mobile.length >= 9 && messgae ){
    $('#form-fill').hide();
...
function hideNewsLetter() {
  ...
  if(email) {
    ...
    $('#footer-newsletter').append(messageHtml);
  }
}
...
$('#newsForm').submit(function(e) {
  ...
  if(email) {
    hideNewsLetter();
    $.ajax({
        url: "https://script.google.com/macros/s/AKfycbze1PMJLjdLZ_EOTl4gC08yiwPhEXlh2Co_j9-TYw/exec",
        data: $(this).serialize(),
        type: "POST",
        dataType: "xml",
